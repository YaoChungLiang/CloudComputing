...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        if dpid == 1: # switch S1
            ...
            match = parser.OFPMatch(eth_type = ether.ETH_TYPE_IP, ipv4_src = '10.0.0.3', ipv4_dst = '10.0.0.1', ip_proto = inet.IPPROTO_TCP)
    ...
    def _packet_in_handler(self, ev):
        ...
        dpid = datapath.id
        self.mac_to_port.setdefailt(dpid,{})
        ...
        out_port = self._find_port(datapath,pkt,dst)
        if out_port is None:
        ...
        if pkt_tcp:
            self.logger.info(pkt_tcp.dst_port)
        if pkt_tcp and pkt_tcp.dst_port == 80:
            self._handle_http(datapath,in_port, out_port,eth_pkt,pkt_ip,pkt_tcp,pkt,buffer_id)
            return
        if pkt_ip:
            match = parser.OFPMatch(in_port = in_port,eth_type = ether.ETH_TYPE_IP,eth_dst = dst,ip_proto = pkt_ip.proto)
    ...
    def _handle_arp(self , datapath , port, pkt_ethernet , pkt_arp):
        ...
        pkt.add_protocol(arp.arp(opcode=arp.ARP_REPLY, src_mac=self.hw_addr , src_ip=self.ip_addr , dst_mac=pkt_arp.src_mac , dst_ip=pkt_arp.src_ip))
